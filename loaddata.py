...
with open('opponents_2018_V1.csv', 'r') as f1:
    ...
    for rows in myReader:
        ...
        if rows[0] not in match:
            match[rows[0]]=[rows[1]]
        if rows[0] in match and rows[1] not in match[rows[0]]:
            match[rows[0]].append(rows[1])
            
        if rows[1] not in match:
            match[rows[1]]=[rows[0]]
        if rows[1] in match and rows[0] not in match[rows[1]] :
            match[rows[1]].append(rows[0])
...
with open('GAME_VARIABLES_2018_V1.csv', 'r') as f2:
    ...
    season0=[]
    for rows in myReader:
        season0.append((rows[0],rows[1],rows[2],rows[3],rows[4]))
season=tuplelist(season0)
...
for i in teamin:
    for t in teamin:
        if teamin[i][1]==teamin[t][1] and teamin[i][0]==teamin[t][0] :
            if i not in divmem and i!=t:
                divmem[i]=[t]
            if i in divmem and i!=t and t not in divmem[i]:
                divmem[i].append(t) 
